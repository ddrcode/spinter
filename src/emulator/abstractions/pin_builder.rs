...
mod tests {
    ...
    fn test_pin_builder() {
        ...
        assert_eq!("A", pins[8].group_name().unwrap());
        assert_eq!("A11", pins[19].name());
        assert_eq!("VSS", pins[20].name());
        assert_eq!("A12", pins[21].name());
        assert_eq!("A15", pins[24].name());
        assert_eq!("D7", pins[25].name());
        assert_eq!("D0", pins[32].name());
