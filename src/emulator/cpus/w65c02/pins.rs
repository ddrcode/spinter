...
impl W65C02_Pins {
    pub fn new() -> Self {
        ...
        let data = pins[25..33].to_vec().into_iter().rev().collect();
    ...
    pub fn set_sync(&self, val: bool) -> &Self {
        self.pins[6].write(val).unwrap();
        self
    }

    pub fn set_data_direction(&self, dir: PinDirection) -> &Self {
        self.pins[33].write(dir == Input).unwrap();
        self.data.set_direction(dir).unwrap();
        self
    }
}
...
impl Index<&str> for W65C02_Pins {
    type Output = Pin;

    fn index(&self, name: &str) -> &Self::Output {
        self.by_name(name)
            .expect(&format!("Pin '{name}' not found"))
    }
}
